...
while True:
    ...
    # hall의 정보를 담은 list
    halls = [(0, 0), (127, 0), (254, 0), (0, 127), (127, 127), (254, 127)]

    # whiteBall_x, whiteBall_y: 흰 공의 X, Y좌표를 나타내기 위해 사용한 변수
    whiteBall_x = balls[0][0]
    whiteBall_y = balls[0][1]

    # target의 순서를 정하기 위한 정렬용 list
    target_lst = []

    # target의 순서를 정함
    for num in order_balls:
        if list(map(int, balls[num])) == [-1, -1]:
            continue
        ...
        temp_distance = math.sqrt(width ** 2 + height ** 2)

        target_lst.append((num, temp_distance))
    ...
    # target_lst가 비어있을 경우: 8번구를 target으로
    else:
        if list(map(int, balls[5])) != [-1, -1]:
            target = 5
