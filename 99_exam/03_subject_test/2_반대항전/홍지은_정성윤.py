...
while True:
    ...
    if order == 1: # 선공일 때 targeting 할 공들
        target_balls = [1, 3, 5]
    elif order ==2: # 후공일때 targeting 할 공들
        target_balls = [2, 4, 5]


    for i in target_balls: # targeting 할 공들 순서대로 target
        if (balls[i][0] > 0) or (balls[i][1] > 0):
    ...
    # 1사분면의 경우
        ...
        if height == 0:
            height = 1
        radian = math.atan(width / height)
        angle = (180 / math.pi * radian) + 360
        
    # distance: 두 점(좌표) 사이의 거리를 계산
    # distance = math.sqrt(width**2 + height**2)
    power = math.sqrt(width**2 + height**2)
