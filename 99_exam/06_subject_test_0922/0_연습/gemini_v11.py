...
def determine_fire_direction(current_pos, target_pos):
    ...
    if current_pos[1] < target_pos[1]: return 'R'
    if current_pos[1] > target_pos[1]: return 'L'
    if current_pos[0] < target_pos[0]: return 'D'
    if current_pos[0] > target_pos[0]: return 'U'
    return my_tank['direction']
...
while game_data is not None:
    ...
    for row in range(len(map_data)):
        for col in range(len(map_data[0])):
            if map_data[row][col] in ['R', 'X', 'E', 'A', 'H', 'C']:
