...
def get_bus_data(date):
    ...
    for row in root.findall('.//row'):
        bus_data.append({
            ...
            '정류장명': row.find('SBWY_STTN_NM').text if row.find('SBWY_STTN_NM') is not None else '',  # row에서 'STTN_NM' 태그의 텍스트 추출)
...
def get_subway_data(date):
    ...
    url = f"{SUBWAY_URL}{SUBWAY_API_KEY}/xml/CardSubwayNew/1/1000/{date}"  # SUBWAY_URL과 SUBWAY_API_KEY, date를 이용하여 API 요청 URL 생성)
    ...
    for row in root.findall('.//row'):
        subway_data.append({
            ...
            '역명': row.find('SBWY_STTN_NM').text if row.find('SBWY_STTN_NM') is not None else '',  # row에서 'SBWY_STNS_NM' 태그의 텍스트 추출)
